  ...
  public Store() {
    ...
    do {
      ...
      if (orderQuantity <= itemsInStore.get(productId - 1).quantity) {
        ...
        System.out.println("Total cost to buy " + orderQuantity + " " + itemsInStore.get(productId - 1).productName
            + " " + totalCost + " $");
    ...
    System.out.println("Enter Manager credentials to view the history");
    Scanner sc = new Scanner(System.in);
    ...
    if (userName.toLowerCase().equals("charu") && password.equals("store123")) {
      for (int i = 0; i < purchaseHistoryList.size(); i++) {
        System.out.println("Purchased " + purchaseHistoryList.get(i).productName + " : "
            + purchaseHistoryList.get(i).quantity + " : " + purchaseHistoryList.get(i).puchaseTime);
      }

    }
    else {
      System.out.println("Wrong credentials!! You don't have access to this");
    }
